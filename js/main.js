...
$(document).ready(function () {
	...
	for (var k = 0; k < 57; k++) {
		$(".appear > img")
			.eq(k)
			.css({ animation: "ani 2.75s linear " + k * 0.05 + "s 1 normal" });
	}
	...
	for (var k = 0; k < 82; k++) {
		$(".loop > img")
			.eq(k)
			.css({
				animation: "ani 4.1s linear " + (2.85 + k * 0.05) + "s infinite normal",
			});
	}
	...
	$("span.srch_open").click(function () {
		...
		if ($(this).hasClass("on")) {
			$(this).children("a").attr("title", "검색 닫기");
		} else {
			$(this).children("a").attr("title", "검색 열기");
		}
	});
	...
	$(".next").click(function () {
		...
		$(".banner_frame")
			.stop()
			.animate({ left: $banNum * -$banner_w }, 600, "linear", function () {
				if ($(".banner_frame > section").eq($banNum).hasClass("white")) {
					//class white를 가지고 있냐?
	...
	var $banner = $(".banner_roll a").click(function () {
		...
		$(".banner_frame")
			.stop()
			.animate({ left: $bnnNum * -$banner }, 600, "linear", function () {
		...
		function () {
			...
			for (var i = 0; i < 20; i++) {
				$(this)
					.find("img")
					.eq(i)
					.css({ animation: "ani 1s linear " + i * 0.05 + "s 1 normal" });
			}
		},
		function () {
			// 마우스를 뗏을때
    ...
    */
	...
	$(window).scroll(function () {
		...
		$(".combine_Left").css({ top: 1526 - scroll * 0.1 });
